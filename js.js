...
function kreni() {
    ...
    for (let i = 0; i < 42; i++) {
        if (tabela[i] != null) {
            if(tabela[i] == danas && today.getFullYear() == datum.getFullYear() && today.getMonth() == datum.getMonth()){
                document.getElementById("main").innerHTML += '<div class="polje danas"><p>' + tabela[i] +'</p></div>';
            }else{
